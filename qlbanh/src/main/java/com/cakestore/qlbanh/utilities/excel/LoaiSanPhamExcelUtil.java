...
public class LoaiSanPhamExcelUtil extends ExcelUtil {

    private static final String[] EXCEL_EXTENSIONS = { "xls", "xlsx", "xlsm" };
    private static final Logger LOGGER = Logger.getLogger(LoaiSanPhamExcelUtil.class.getName());

    public static List<LoaiSanPhamModel> readLoaiSanPhamModelsFromExcel() throws IOException {
        ...
        if (option == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                List<LoaiSanPhamModel> LoaiSanPhamModel = convertToLoaiSanPhamModelList(data);
                ...
                LOGGER.log(Level.SEVERE, "Error occurred while converting data to LoaiSanPhamModel: " + e.getMessage());
    ...
    private static List<LoaiSanPhamModel> convertToLoaiSanPhamModelList(List<List<String>> data) {
        List<LoaiSanPhamModel> LoaiSanPhamModels = new ArrayList<>();
        for (List<String> row : data) {
            String maLoaiSP = row.get(0);
            String tenLoaiSP = row.get(1);
            String Mota = row.get(2);

            LoaiSanPhamModel model = new LoaiSanPhamModel(maLoaiSP, tenLoaiSP, Mota);
            LoaiSanPhamModels.add(model);
            LoaiSanPhamBUS.getInstance().addModel(model);
        }

        return LoaiSanPhamModels;
    }

    public static void writeLoaiSanPhamModelsToExcel(List<LoaiSanPhamModel> LoaiSanPhamModels) throws IOException {
        List<List<String>> data = new ArrayList<>();

        // Create header row
        List<String> headerValues = Arrays.asList("Mã loại", "Tên loại");
        data.add(headerValues);

        // Write data rows
        for (LoaiSanPhamModel LoaiSanPhamModel : LoaiSanPhamModels) {
            List<String> values = Arrays.asList(
                    String.valueOf(LoaiSanPhamModel.getMaLoaiSP()),
